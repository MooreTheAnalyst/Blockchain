...
function deriveAccounts(root, count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const account = {
      path,
      privateKey: Buffer.from(child.privateKey).toString("hex"), // Explicitly handle the privateKey buffer
      publicKey: Buffer.from(child.publicKey).toString("hex"), // Explicitly handle the publicKey buffer
    };
...
const blockchainAccounts = generateBlockchainAccounts(5);
